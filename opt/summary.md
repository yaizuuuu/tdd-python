...
テスト駆動開発とは個人のスキルである。
コードの書き方のスタイルであり、コードとの向き合い方である。

### テストは質をあげない - 上げるのはプログラミング

プロダクトの質を上げるのはテストではなく、プログラミング。
テストを書いて、設計が改善しないのでは、ただの回帰テストでしかない。


**テスト駆動開発における質の向上の手段は、リファクタリングによる継続的でインクリメンタルな設計である。**

**テスト駆動により生み出されたコードが最初に考えていた設計からどのくらい変わったか、そこに驚きがあったか、それは良い変化だったかを注意深く観察する。**

**テスト駆動開発により、設計が良い方向に変わり、コードが改善され続け、開発者が前向きになることが、目指すべきゴールである。**
